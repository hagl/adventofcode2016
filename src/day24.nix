...
let 
  ...
  step = acc@{g, a, ps}: n:
    ...
    in deepSeq (nbs) (step (update g nbs n a) (n+1));

  findDistances = grid: maxNum: start: j:
    let
      a = genList (x: 0) maxNum;
  ...
  solve = file: maxNum:
    let
      ...
      distances = map (j: findDistances grid (maxNum + 1) (elemAt positions j)  j) (range 0 maxNum);
